...
exports.deleteCrop = exports.updateCropStatus = exports.createCrop = exports.getCrops = exports.mockCrops = void 0;
exports.mockCrops = [
    {
        id: 'crop-101',
        farmerId: 'f-101',
...
const getCrops = async (req, res) => {
    ...
    let filtered = [...exports.mockCrops];
    if (search) {
        const q = String(search).toLowerCase();
        filtered = filtered.filter(c => c.cropName.toLowerCase().includes(q) ||
...
const createCrop = async (req, res) => {
    ...
    exports.mockCrops.unshift(newCrop);
...
const updateCropStatus = async (req, res) => {
    ...
    const crop = exports.mockCrops.find(c => c.id === id);
    if (!crop) {
        return res.status(404).json({ success: false, error: 'Crop record not found' });
    }
    if (status)
        crop.status = status;
    if (actualYieldTon !== undefined)
        crop.actualYieldTon = parseFloat(actualYieldTon);
    if (harvestDate)
        crop.harvestDate = harvestDate;
    return res.json({ success: true, message: 'Crop status updated', data: crop });
};
exports.updateCropStatus = updateCropStatus;
const deleteCrop = async (req, res) => {
    const index = exports.mockCrops.findIndex(c => c.id === req.params.id);
    if (index === -1) {
        return res.status(404).json({ success: false, error: 'Crop record not found' });
    }
    exports.mockCrops.splice(index, 1);
