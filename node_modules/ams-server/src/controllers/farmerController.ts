...
export const createFarmer = async (req: Request, res: Response) => {
  ...
  const existing = mockFarmers.find(f => f.nationalId === nationalId);
  if (existing) {
...
export const updateFarmer = async (req: Request, res: Response) => {
  const index = mockFarmers.findIndex(f => f.id === req.params.id);
  if (index === -1) {
    return res.status(404).json({ success: false, error: 'Farmer not found' });
  }
  ...
  return res.json({ success: true, message: 'Farmer updated successfully', data: mockFarmers[index] });
};
